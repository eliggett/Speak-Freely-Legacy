...
extern void analys(float speech[], int voice[2], int *pitch, float *rms, float rc[]);
extern void bsynz(float coef[], int ip, int iv, float sout[],
...
extern void difmag(float speech[], int tau[], int ltau, int maxlag, float amdf[], int *minptr, int *maxptr);
extern void dyptrk(float amdf[], int minptr, int voice, int *pitch, int *midx);
extern void encode(int voice[2], int *pitch, float *rms, float rc[ORDER], int *ipitch, int *irms, int irc[ORDER]);
...
extern void irc2pc(float rc[MAXORD][11], float pc[], float gprime, float *g2pass, int where);
...
extern int round(double afloat);
extern void rcchk(float rc1f[MAXORD][AF]);
extern void synths(int voice[], int *pitch, float *rms,
    	    	   float rc[], float speech[], int *k );
extern void tbdm(float speech[], int tau[], float amdf[], int *minptr, int *maxptr, int *mintau);	    		   
extern void voicin(int vwin[2][AF], float *inbuf, float *lpbuf, int half, float minamd,
    	    float maxamd, int mintau, float ivrc[2], int *obound, int voibuf[2][AF+1]);
#ifdef NN_VOICE
extern void voicing(float speech[], float lpspeech[], int start, int end, int pitch, int *v1, int *v2);
#endif
extern void vparms(int *vwin, float *inbuf, float *lpbuf, int half, float *dither,
